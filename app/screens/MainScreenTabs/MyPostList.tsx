...
const GeneralPostList: React.FC<Props> = ({
  ...
  const getNextPage = async () => {
    const collection = postState.myPosts;
    if (collection.currentPage < collection.lastPage) {
      try {
        setLoading(true)
        const nextPage = collection.currentPage+1;
        const newData = await PostService.getOwnedPostList(nextPage);
        if (newData && newData.data.length) {
          ...
          updatePosts({
            generalPosts: postState.generalPosts,
            myPosts: nextData
          });
        }
      } catch (e) {
  ...
  const refreshList = async () => {
    try {
      setLoading(true);
      const data = await PostService.getOwnedPostList();
      updatePosts({
        generalPosts: postState.generalPosts,
        myPosts: data
      });
